...
<html xml:lang="${cmsfn.language()}" lang="${cmsfn.language()}">
  <head>
    ...
    [#-- To load resources you can link them manually (e.g. line below) --]
      [#-- <link rel="stylesheet" type="text/css" href="${ctx.contextPath}/.resources/argos-test-lm/webresources/css/bootstrap.css" media="all" /> --]
      [#-- <script src="${ctx.contextPath}/.resources/argos-test-lm/webresources/js/jquery.js"></script> --]
    [#-- or via theme --]
    ...
    [#-- uncomment next line to use resfn templating functions to load all css which matches a globbing pattern --]
      [#-- ${resfn.css(["/argos-test-lm/.*css"])!} --]
  </head>
  <body class="argos-test-page ${cmsfn.language()}">
